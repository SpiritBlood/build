...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^spirit_") ; then
       SPIRIT_BUILD=$(echo -n $1 | sed -e 's/^spirit_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $SPIRIT_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       SPIRIT_BUILD=
    fi
    export SPIRIT_BUILD
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var SPIRIT_VERSION)
        ZIPFILE=spirit-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $SPIRIT_FIXUP_COMMON_OUT ]; then
